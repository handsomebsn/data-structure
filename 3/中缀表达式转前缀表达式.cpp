 ...
 *返回值为0表示获得的是整形操作数 
 */  
 ElemType getNext(Stack &biaodashi,ElemType &n){  
    char c;  
    n=0;  
    c=Pop(biaodashi);
    if(In(c)){            /*是算符*/  
...
void fun(){
    ElemType c,theta;
    Stack biaodashi;InitStack(biaodashi);
    Stack zbiaodashi;InitStack(zbiaodashi);
    Push(biaodashi,'#');c=getchar();
    while(c!='#')
    {
        Push(biaodashi,c);
        c=getchar();
    ...
    //Stack opnd;InitStack(opnd);

    int flag=getNext(biaodashi,c);
    while(c!='#'||GetTop(optr)!='#'){
        if(!flag){Push(zbiaodashi,c);flag=getNext(biaodashi,c);}
        else
            switch(Precede(c,GetTop(optr))){
                case '0':printf("biaodashi is wrong\n");exit(-1);
                case '>':Push(optr,c);flag=getNext(biaodashi,c);break;
                case '=':Pop(optr);flag=getNext(biaodashi,c);break;
                case '<':Pop(optr,theta);Push(zbiaodashi,theta); break;

            }
    }
  
    while(!StackEmpty(zbiaodashi))
        printf("%c",Pop(zbiaodashi));

    DestroyStack(optr);
    DestroyStack(biaodashi);
    DestroyStack(zbiaodashi);
